# Site Navigation Structure

## Main Navigation

- Home (`/`)
- What We Do (`/what-we-do`)
  - Digital Engineering & Manufacturing (`/what-we-do#digital-engineering-manufacturing`)
  - Maintenance & Logistics Optimization (`/what-we-do#maintenance-logistics-optimization`)
  - Enterprise IT & Transformation (`/what-we-do#enterprise-it-transformation`)
  - Data & Analytics (`/what-we-do#data-analytics`)
  - Program Support (`/what-we-do#program-support`)
- Solutions (removed — content retained on home page without links)
- Contracts (`/contracts`)
- Jobs (`/jobs`)
- Contact Us (`/contact-us`)

## Horizonal Scrolling Tickers

- Clients (display client logos in continuous horizonatal scrolling left to right, from the '/brand_assets/clients' folder)
- Partners (display partner logos in continuous horizontal scrolling from right to left, from the '/brand_assets/partners' folder)

